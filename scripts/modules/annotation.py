...
def run_blast(df):
        ...
        for i in range(df.shape[0]):
                ...
                output = subtype+";"+str(match_percent)+";"+str(overlap_insert_start)+";"+str(overlap_insert_end)+";"+str(overlap_te_start)+";"+str(overlap_te_end)
...
def get_cigar(df):
        ...
        for i in range(df.shape[0]):
                ...
                out_ref = open("../tmp/bwa/tmp.fa", 'w')
                out_ref.write(">tmp" + "\n")
                out_ref.write(str(ref_seq))
                out_ref.close()
                os.system("bwa index ../tmp/bwa/tmp.fa")

                out_read = open("../tmp/bwa/tmp.fq", 'w')
                out_read.write(">left" + "\n")
                ...
                command = "bwa mem -v 1 ../tmp/bwa/tmp.fa ../tmp/bwa/tmp.fq"
                result = subprocess.run(command, shell=True, stdout=subprocess.PIPE, text=True)
                df_tmp = pd.DataFrame([row.split("\t") for row in result.stdout.split("\n")])
